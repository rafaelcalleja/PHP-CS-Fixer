...
class YodaConditionsFixerTest extends AbstractFixerTestBase
{
    ...
    public function provideExamples()
    {
        return array(
            array(
                '<?php return $a[0 === $b ? $c : $d] === count($this->array[$var]);',
                '<?php return $a[$b === 0 ? $c : $d] === count($this->array[$var]);',
            ),
            array(
                '<?php return count($this->array[$var]) === $a[0 === $b ? $c : $d];',
                '<?php return count($this->array[$var]) === $a[$b === 0 ? $c : $d];',
            ),
            array(
                '<?php return false === method($this->property());',
                '<?php return method($this->property()) === false;',
            ),
            array(
                '<?php return true === is_null($a[0]);',
                '<?php return is_null($a[0]) === true;',
            ),
            array(
                '<?php return true == is_a($object, $element);',
                '<?php return is_a($object, $element) == true;',
            ),
